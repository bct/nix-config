{ pkgs, ... }:
let
  colours-gruvbox = pkgs.writeText "colors-gruvbox-shuber.muttrc" ''
...
in
{
  home-manager.users.bct = {
    xdg.configFile."neomutt/muttrc".text = ''
      set from = "Brendan Taylor <bct@diffeq.com>"

      set mbox_type=Maildir
      set folder=/var/vmail/diffeq.com/bct/mail/
      set record=+Sent
      set spoolfile=+	# where my new mail is located

      mailboxes ! +Junk +catchall +lists
