...
let
  repos = {
    ...
    lubelogger = {
      pubKey = "lubelogger";
      path = "lubelogger.domus.diffeq.com";
    };

    mail = {
      pubKey = "mail";
      path = "mail.domus.diffeq.com";
    };

    medley = {
      pubKey = "medley";
      path = "medley.domus.diffeq.com";
    };
  };
in
{
  ...
  services.borgbackup.repos =
    (lib.mapAttrs (name: { pubKey, path }: {
      authorizedKeys = [
        (builtins.readFile (config.diffeq.secretsPath + /ssh/borg-${pubKey}.pub))
      ];
      path = "/srv/borg/${path}";
    }) repos)
    // {
